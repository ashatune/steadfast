...
struct PanicResetView: View {
    ...
    private func startBreathingLoop() {
        ...
        breathTimer = Timer.scheduledTimer(withTimeInterval: 4, repeats: false) { _ in
            ...
            breathTimer = Timer.scheduledTimer(withTimeInterval: 6, repeats: true) { _ in
                animateBreath(to: .inhale)
    ...
    private func setupAndPlayAudio() {
        ...
        guard let url = Bundle.main.url(forResource: "SteadfastSOSbodyScan", withExtension: "wav") else {
            return
        }
